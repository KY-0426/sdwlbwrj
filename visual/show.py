...
def save_fig(batch, i, fmt = 'png'):
    if os.path.exists(f"../{fmt}/{batch}") == 0:
        os.makedirs(f"../{fmt}/{batch}", exist_ok=True)
    plt.savefig(f'../{fmt}/{batch}/{i}.png', dpi=50, bbox_inches='tight')
...
def save_heat(batch, i, heat, fmt = 'png'):
    if os.path.exists(f"../{fmt}/{batch}") == 0:
        os.makedirs(f"../{fmt}/{batch}", exist_ok=True)
    # plt.savefig(f'../{fmt}/{batch}/{i}.png', dpi=50, bbox_inches='tight')
    plt.imsave(f'../{fmt}/{batch}/{i}.png',heat[i])
